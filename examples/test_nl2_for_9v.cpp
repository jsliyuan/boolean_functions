...
int main() {
  ...
  */
  ...
  BooleanFun f_quard(4);
  f_quard.set_anf("x1x2+x3x4+x1x4+x3+1");
  assert(float_equals(f_quard.Gowers_norm_u3(),1.00));
  f_quard=f_quard.derivative(7);
  assert(f_quard.get_degree()==1);

  BooleanFun f_const(6,"1");
  assert(float_equals(f_const.Gowers_norm_u2(),1.00));
  assert(float_equals(f_const.Gowers_norm_u3(),1.00));

  BooleanFun f_affine(7);
  ...
  BooleanFun f_quard1(5,"x1x5+x2x3+x4x5+x2x5+x3x4");
